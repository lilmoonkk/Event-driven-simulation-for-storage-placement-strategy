...
pygame.display.set_caption("Dynamic partition-best fit") 
win.fill(purple)
...
outfile=open('Dynamic partition-best fit.txt','w')
totaljobdone=0
        ...
        if eventlist[0].eventtype=='Arrive':
            ...
            if joblist[0].jobsize>maxmemory:
                outfile.write("J"+str(joblist[0].jobno)+
                              ": There is no suitable memory blaock for this job at all\n")
                ...
                #sort
                for iteration in range(1,len(temporary)):
                    for j in range(0,len(temporary)-iteration):
                        if temporary[j].size>temporary[j+1].size:
